...
export const enquirySchema = z
  .object({
    ...
    course: z
      ...
      .max(100)
      .refine(
        (value) => value === "not-sure" || programmes.some((p) => p.slug === value),
...
export async function saveEnquiry(data: z.infer<typeof enquirySchema>, ip: string) {
  ...
  const bucket = createHash("sha256")
    .update(`${process.env.ENQUIRY_RATE_SALT}:${ip}:${hour}`)
    .digest("hex");
  const client = await db.connect();
  try {
    await client.query("BEGIN");
    const limited = await client.query(
      `INSERT INTO enquiry_rate_limits(bucket,attempts,expires_at)
      VALUES($1,1,now()+interval '2 hours') ON CONFLICT(bucket) DO UPDATE
      SET attempts=enquiry_rate_limits.attempts+1 RETURNING attempts`,
      [bucket],
    );
    if (limited.rows[0].attempts > 10) {
      await client.query("COMMIT");
      return { limited: true, created: false };
    }
  ...
  try {
    const response = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        ...
        to: [ENQUIRY_TO_EMAIL],
        subject: "New Beyond Fluency Lab enquiry",
    ...
    /* The enquiry remains saved even if the provider is unavailable. */
  }
