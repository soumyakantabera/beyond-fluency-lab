...
import refundsMd from "@/content/legal/refunds.md?raw";
import ukMd from "@/content/legal/uk-privacy-addendum.md?raw";
import usMd from "@/content/legal/us-privacy-addendum.md?raw";

type PolicyKind = "terms" | "privacy" | "cookies" | "refunds" | "gdpr" | "uk" | "us";

function headings(md: string) {
  return [...md.matchAll(/^## (.+)$/gm)].map((match) => match[1]);
}
function titleOf(md: string) {
  return md.match(/^# (.+)$/m)?.[1] ?? "";
}
function summaryOf(md: string) {
  const plain = md.replace(/^# .+$/m, "").replace(/^## .+$/gm, "").replace(/[*_\[\]]/g, " ").replace(/\s+/g, " ").trim();
  return plain.slice(0, 180).replace(/\s+\S*$/, "") + ".";
}

const drafted: Record<"terms" | "privacy" | "refunds" | "uk" | "us", string> = {
  terms: termsMd,
  privacy: privacyMd,
  refunds: refundsMd,
  uk: ukMd,
...
const meta: Record<PolicyKind, { title: string; summary: string; sections: string[] }> = {
  terms: { title: titleOf(termsMd), summary: summaryOf(termsMd), sections: headings(termsMd) },
  privacy: { title: titleOf(privacyMd), summary: summaryOf(privacyMd), sections: headings(privacyMd) },
  ...
  refunds: { title: titleOf(refundsMd), summary: summaryOf(refundsMd), sections: headings(refundsMd) },
  ...
  uk: { title: titleOf(ukMd), summary: summaryOf(ukMd), sections: headings(ukMd) },
  us: { title: titleOf(usMd), summary: summaryOf(usMd), sections: headings(usMd) },
};
...
function inline(value: string) {
  ...
  text = text.replace(/\[Cookie Policy\]/g, '<a href="/legal/cookies">Cookie Policy</a>');
  text = text.replace(/\[Refund & Cancellation Policy\]/g, '<a href="/legal/refunds">Refund & Cancellation Policy</a>');
  text = text.replace(/\[([^\]]+)\]\(([^)]+)\)/g, '<a href="$2">$1</a>');
  text = text.replace(/\*\*(.+?)\*\*/g, "<strong>$1</strong>");
  text = text.replace(/(^|[^*])\*(?!\s)(.+?)\*(?!\*)/g, "$1<em>$2</em>");
  text = text.replace(/`([^`]+)`/g, "<code>$1</code>");
  text = text.replace(/\b([\w.+-]+@[\w.-]+\.[A-Za-z]{2,})\b/g, '<a href="mailto:$1">$1</a>');
  text = text.replace(/\b(ico\.org\.uk)\b/g, '<a href="https://$1" rel="noopener noreferrer">$1</a>');
  text = text.replace(/\+91 9674479949/g, '<a href="tel:+919674479949">+91 9674479949</a>');
  return text;
}
function mdToHtml(md: string) {
  ...
  for (const line of lines) {
    ...
    if (line.startsWith("## ")) {
      ...
      html.push(`<section id="${id(heading)}"><h2>${escapeHtml(heading)}</h2>`);
      open = true;
      continue;
    }
...
function Draft({ kind }: { kind: keyof typeof drafted }) {
  return <div className="policy-source" dangerouslySetInnerHTML={{ __html: mdToHtml(drafted[kind]) }} />;
}
...
function Gdpr() { return <>
  ...
  <Section title="EU representative"><p>Currently being appointed. Until this is complete, EU data subjects may contact the data controller directly using the details below.</p><p>Data controller: Learn With Smile, a sole proprietorship in India, trading as Beyond Fluency Lab · 75/2/4, Raja Ram Mohan Roy Road, Kolkata 700008, India · <a href="mailto:info@learnwithsmile.app">info@learnwithsmile.app</a>.</p></Section>
...
export function LegalPage({ kind }: { kind: PolicyKind }) {
  ...
  const pages: Record<PolicyKind, ReactNode> = {
    terms: <Draft kind="terms" />,
    privacy: <Draft kind="privacy" />,
    cookies: <Cookies />,
    refunds: <Draft kind="refunds" />,
    gdpr: <Gdpr />,
    uk: <Draft kind="uk" />,
    us: <Draft kind="us" />,
  };
  return (
    <main id="main">
      <PageHero eyebrow="LEGAL" title={policy.title} description={policy.summary} />
      <section className="wrap section policy-layout">
        <PolicyNav kind={kind} />
        <article className="policy-copy prose">
          <div className="policy-meta"><span>Last updated 11 September 2026</span></div>
          {pages[kind]}
        </article>
      </section>
    </main>
