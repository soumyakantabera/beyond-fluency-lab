...
export function pageSchema(path: string, title: string, description: string) {
  ...
  const graph: Record<string, unknown>[] = [
    ...
    {
      "@type":
        path === "/courses" || path === "/blog" || path === "/who-its-for"
          ? "CollectionPage"
          : "WebPage",
      "@id": url + "#webpage",
      url,
      name: title.replace(/ \| Beyond Fluency Lab$/, ""),
      description,
  ...
  if (path !== "/") {
    ...
    const crumbs = parts.flatMap((part, i) =>
      part === "legal"
        ? []
        : [
            {
              ...
              name:
                labels[part] ||
                courses.find((c) => c.slug === part)?.name ||
                segments.find((s) => s.slug === part)?.name ||
                (i === parts.length - 1 ? title.split(" | ")[0] : part),
    ...
    graph.push({
      "@type": "BreadcrumbList",
      itemListElement: [{ name: "Home", item: ORIGIN + "/", "@type": "ListItem" }, ...crumbs].map(
        (item, i) => ({ ...item, position: i + 1 }),
      ),
    });
  }
  if (path === "/courses")
  ...
  if (article)
    graph.push({
      ...
      ...(article.image ? { image: ORIGIN + "/assets/" + article.image + ".webp" } : {}),
      description: article.description,
      articleSection: article.category,
      inLanguage: "en",
      datePublished: article.published || "2026-09-06",
      author: {
        "@type": "Organization",
        ...organization,
        name: "Beyond Fluency Lab",
